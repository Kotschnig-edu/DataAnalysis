...
\begin{itemize}[leftmargin=*]
    \item Dataset name and source (Kaggle, Hugging Face, Westermo tests, etc.):
    \item Why it is suitable for time-series analysis:
    \item Time period covered and sampling frequency:
    \item Key variables analyzed (signals, sensors, physical quantities):
    \item Size and structure:
    \begin{itemize}
        \item Number of observations (rows):
        \item Number of features (columns):
        \item Target variable(s) if any:
    \end{itemize}
